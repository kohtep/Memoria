...
{
	hde_uint8_t x, c, *p = (hde_uint8_t *)code, cflags, opcode, pref = 0;
	hde_uint8_t *ht = hde64_table, m_mod, m_reg, m_rm, disp_size = 0;
	hde_uint8_t op64 = 0;
  ...
  pref_done:
	...
	if (cflags & C_GROUP) {
		hde_uint16_t t;
		t = *(hde_uint16_t *)(ht + (cflags & 0x7f));
		cflags = (hde_uint8_t)t;
		x = (hde_uint8_t)(t >> 8);
	}
	...
	if (cflags & C_MODRM) {
		...
		if (pref & PRE_LOCK) {
			if (m_mod == 3) {
				...
				hde_uint8_t *table_end, op = opcode;
				if (hs->opcode2) {
	  ...
	  no_error_operand:
		...
		switch (disp_size) {
			...
			case 2:
				hs->flags |= F64_DISP16;
				hs->disp.disp16 = *(hde_uint16_t *)p;
				break;
			case 4:
				hs->flags |= F64_DISP32;
				hs->disp.disp32 = *(hde_uint32_t *)p;
				break;
		}
		p += disp_size;
	...
	if (cflags & C_IMM_P66) {
		...
		if (op64) {
			hs->flags |= F64_IMM64;
			hs->imm.imm64 = *(hde_uint64_t *)p;
			p += 8;
		} else if (!(pref & PRE_66)) {
			hs->flags |= F64_IMM32;
			hs->imm.imm32 = *(hde_uint32_t *)p;
			p += 4;
		} else
			goto imm16_ok;
	}
...
disasm_done:
	...
	if ((hs->len = (hde_uint8_t)(p - (hde_uint8_t *)code)) > 15) {
